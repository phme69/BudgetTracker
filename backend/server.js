...
app.post('/customer/register', (req, res) => {
    const { customer_name, email, username, password, phone_number, address, customer_image } = req.body;
    ...
    db.query(query, [customer_level, customer_name, email, username, password, phone_number, address, customer_image], (err, results) => {
        if (err) {
...
app.post('/seller/register', (req, res) => {
    const { username, password, seller_name, email, phone_number, address, seller_image } = req.body;
    const query = `
        INSERT INTO Sellers (username, password, seller_name, email, phone_number, address, seller_image)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
    db.query(query, [username, password, seller_name, email, phone_number, address, seller_image], (err, results) => {
        if (err) {
...
app.post('/products', (req, res) => {
    const { category, title, price, stock_quantity, brand, max_discountable_price, description, image_url } = req.body;
    const query = 'INSERT INTO Products (category, title, price, stock_quantity, brand, max_discountable_price, description, image_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
    db.query(query, [category, title, price, stock_quantity, brand, max_discountable_price, description, image_url], (err, results) => {
        if (err) {
...
app.put('/products/:product_id', (req, res) => {
    const { product_id } = req.params;
    const { category, title, price, stock_quantity, brand, max_discountable_price, description, image_url } = req.body;
    const query = 'UPDATE Products SET category = ?, title = ?, price = ?, stock_quantity = ?, brand = ?, max_discountable_price = ?, description = ?, image_url = ? WHERE product_id = ?';
    db.query(query, [category, title, price, stock_quantity, brand, max_discountable_price, description, image_url, product_id], (err, results) => {
        if (err) {
...
app.post('/shops', (req, res) => {
    const { shop_name, seller_id, shop_image, area_code, area_name, full_address, shop_rating } = req.body;
    const query = 'INSERT INTO shops (shop_name, seller_id, shop_image, area_code, area_name, full_address, shop_rating) VALUES (?, ?, ?, ?, ?, ?, ?)';
    db.query(query, [shop_name, seller_id, shop_image, area_code, area_name, full_address, shop_rating], (err, results) => {
        if (err) {
...
app.put('/shops/:shop_id', (req, res) => {
    const { shop_id } = req.params;
    const { shop_name, seller_id, shop_image, area_code, area_name, full_address, shop_rating } = req.body;
    const query = 'UPDATE shops SET shop_name = ?, seller_id = ?, shop_image = ?, area_code = ?, area_name = ?, full_address = ?, shop_rating = ? WHERE shop_id = ?';
    db.query(query, [shop_name, seller_id, shop_image, area_code, area_name, full_address, shop_rating, shop_id], (err, results) => {
        if (err) {
...
app.post('/cart', (req, res) => {
    ...
    db.query(checkQuery, [customer_id, product_id], (err, results) => {
        ...
        if (results.length > 0) {
            ...
            db.query(updateQuery, [quantity, customer_id, product_id], (err, result) => {
                if (err) {
            ...
            db.query(insertQuery, [customer_id, product_id, quantity], (err, result) => {
                if (err) {
...
app.put('/customer/:customer_id', (req, res) => {
    const { customer_id } = req.params;
    const { customer_level, customer_name, email, username, phone_number, address, customer_image } = req.body;
    const query = `
        UPDATE Customers
        SET customer_level = ?, customer_name = ?, email = ?, username = ?, phone_number = ?, address = ?, customer_image = ?
        WHERE customer_id = ?
    `;
    db.query(query, [customer_level, customer_name, email, username, phone_number, address, customer_image, customer_id], (err, results) => {
        if (err) {
